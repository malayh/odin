...
def _fake_pipeline(monkeypatch, hits, llm_answer, calls=None):
    async def fake_search_graph(session, owner, query, top_k=10):
...
async def test_answer_grounded_with_citations(monkeypatch):
    doc = uuid.uuid4()
    hit = _hit("Mara founded Helios.", doc_id=doc)
    ...
    out = await answering.answer(None, uuid.uuid4(), "who founded Helios?")
...
async def test_answer_refuses_without_calling_llm_when_no_context(monkeypatch):
    ...
    out = await answering.answer(None, uuid.uuid4(), "anything?")
...
async def test_answer_drops_ungrounded_world_knowledge(monkeypatch):
    ...
    out = await answering.answer(None, uuid.uuid4(), "which way does the sun rise?")
    assert out.confident is False
    assert "knowledge base" in out.text
    assert out.citations == []


async def test_answer_keeps_only_grounded_citations(monkeypatch):
    doc = uuid.uuid4()
    ...
    out = await answering.answer(None, uuid.uuid4(), "q")
